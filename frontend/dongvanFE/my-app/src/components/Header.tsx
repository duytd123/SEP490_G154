...
import logo from "../images/logo.jpg"; // ✅ import trực tiếp từ src/images

type HeaderProps = {
  /** Đường dẫn logo tùy chọn, nếu không truyền sẽ dùng logo mặc định */
  logoSrc?: string;
};
...
export default function Header({ logoSrc }: HeaderProps) {
  ...
  // ✅ Ưu tiên logoSrc được truyền từ props, fallback về ảnh import
  const finalLogo = logoSrc || logo;

  return (
    <header className="w-full shadow sticky top-0 z-50">
      {/* ==== TOP BAR (chỉ hiện khi ở top) ==== */}
      {isTop && (
        <div className="bg-sky-600 text-white text-sm py-2 px-6 flex items-center justify-between transition-all duration-300">
          <div className="flex items-center gap-2">
            ...
            <span>Cần hỗ trợ? Gọi: +84 964173603</span>
          </div>
          ...
          <div className="flex items-center gap-5">
            ...
            >
              <UserIcon className="w-4 h-4" /> <span>Đăng nhập</span>
            </Link>
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-1 hover:underline"
              >
                <UserPlusIcon className="w-4 h-4" /> <span>Đăng ký</span>
              </button>
      ...
      <div className="bg-white flex items-center justify-between px-8 py-4">
        ...
        <nav className="flex gap-6 font-medium text-gray-700">
          ...
          <Link to="/homestays" className="hover:text-sky-600">
            Homestay
          </Link>
